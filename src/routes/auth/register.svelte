<script>
  ...
  const CRSID_PATTERN = /[A-z]{2,6}[0-9]{1,6}/;
...
<form on:submit|preventDefault class:submitted class="theme-{$themeName}">
  ...
  <label for="password">Password<input
      type="password"
      use:checkValid="{{ validityErrors: { tooShort: 'Password should be at least 8 characters long' }, bothPresent: { id: 'password', error: 'Password and password confirmation do not match' } }}"
      bind:value="{password}"
      minlength="8"
      data-test="password"
      required="true"
  ...
  <label for="password-confirm">Confirm password<input
      id="password-confirm"
      type="password"
      use:checkValid="{{ bothPresent: { id: 'password', error: 'Password and password confirmation do not match' } }}"
