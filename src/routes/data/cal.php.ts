...
export async function get({ query, headers }) {
  ...
  const { host, "x-forwarded-proto": proto, "x-forwarded-for": ipAddress } = headers;
  const baseUrl = `${proto}://${host}`;
  if (testAuthLink(query)) {
    client = new GraphQLClient(fetch, {
      domain: process.env["GRAPHQL_REMOTE"],
      headers: { authorization: `Bearer ${graphqlAuthenticationToken(query.get("_cal_uid"))}` },
    });
  } else {
    return {
      status: 401,
  ...
  try {
    switch (query.get("type")) {
      case "gig":
        ...
        body = await singleGig(client, parseInt(query.get("gig")), baseUrl);
        filename = `gig${query.get("gig")}.ics`;
        break;
      case "allgigs":
        body = await allGigs(client, parseInt(query.get("_cal_uid")), ipAddress, baseUrl);
        filename = `allgigs.ics`;
        break;
      case "mygigs":
        body = await myGigs(client, parseInt(query.get("_cal_uid")), ipAddress, baseUrl);
        filename = `mygigs.ics`;
        break;
    }
  } catch (e) {
