Function.prototype.partial = function(...partialArgs) {
  let that = this;
  return (...args) => that(...partialArgs, ...args);
};
...
export const sortLineup = lineup => {
  ...
  for (let person of lineup) {
    person.user_instruments.sort(instrumentsByRank).sort(instrumentsLeadersFirst.partial(leaderInstruments));
  }
  lineup.sort(peopleByInstrumentRank).sort(peopleLeadersFirst);
  return lineup;
};
